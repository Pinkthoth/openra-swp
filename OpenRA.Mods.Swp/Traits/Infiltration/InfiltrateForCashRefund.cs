...
namespace OpenRA.Mods.Swp.Traits
{
	...
	sealed class InfiltrateForCashRefundInfo : TraitInfo
	{
		...
		public override object Create(ActorInitializer init) { return new InfiltrateForCashRefund(this); }
	}

	sealed class InfiltrateForCashRefund : INotifyInfiltrated
	{
		readonly InfiltrateForCashRefundInfo info;

		public InfiltrateForCashRefund(InfiltrateForCashRefundInfo info) { this.info = info; }
		...
		void INotifyInfiltrated.Infiltrated(Actor self, Actor infiltrator, BitSet<TargetableType> types)
		{
			...
			var toGive = infiltrator.GetSellValue() * info.Percentage / 100 * infiltrator.Trait<IHealth>().HP / infiltrator.Trait<IHealth>().MaxHP;

			infiltrator.Owner.PlayerActor.Trait<PlayerResources>().GiveCash(toGive);
